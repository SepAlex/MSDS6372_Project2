---
title: "MSDS 6372 Project 2 - Using classification methods to determine an outcome of a telemarketing campaign."
...
For our analysis, we've used all variables. We separated variables into categorical and continous and examined each group separately. 

By looking at the histograms and boxplots of continous variables in \autoref{fig2} and \autoref{fig3}, we found that _duration_, _balance_, _campaign_ and _previous_ variables might have an impact on our binary respone, while _age_ has no apparant effect on it. Also, we found that _day_ does not seem to have an impact on our dependent variable. 
...
The count plot of the binary response variable in \autoref{fig4} suggests that the data is unbalanced.

\autoref{fig5} show the counts and percentage frequencies of the categorical variables for each factor level by the response variable. It lets us see if a specific level or group of a factor has a higher or lower counts than its counterparts that might contribute to the likelihood of subscribing a term deposit. 

The proportions of those who subscribed to a term deposit seems to vary by job categories even for those with rougly the same sample size. For example, the proportion of subscribing to a term deposit is higher for clients who hold an administrative position and the proportion is lower for individuals who are self-employed. Thus, _job_ is an effect on the likelihood of a client subscribing to a term deposit. 
...
We assume that observations are independent of one another.

### Model Fit

First, the overall test was performed to test the null hypothesis that at least one coefficient is different from 0. Using the Likeliness Ratio test, we reject the null hypothesis at the significant level of 0.05 and conclude that the overall model is significan with the p-value <0.0001 (\autoref{fig19}). 

We then include all the main effects, both categorical and continuous variables, to see which predictors are significant. \autoref{fig20} (left) shows the output with all the main effects and their respective p-values. Based on the results, _education_, _default_, _age_, _balance_, _pdays_, and _previous_ are non-significant at the alpha level of 0.05. Thus, we remove these predictors and refit the model. After that the new output is shown in \autoref{fig20} (right).

### Parameter Interpretation

\autoref{fig21a} displays the coefficient estimates for each factor level and \autoref{fig21b} displays the odd ratio estimates and the confident intervals for each level. Here is our interpretation of a subset of most interesting estimates.  
...
\autoref{fig22} shows the ROC curve of the traning dataset (top) and the ROC curve on the test dataset (bottom). The area under the curve (AUC) is commonly used to assess the prediction performance of the logistics model, the closer it's to 1, the better the prediction is. The AUC based on the training data is 0.9096 and 0.9124 for the test data, which indicates that we did not overfit the model and the predicitibility power of the model is quite high. 
...
related. Discretization of continuous features is a simple but useful way to include additional information that might solve this problem, and we'll pay special attention to those variables that were deemed as non-significant by the baseline model. 
...
Likely due to these limitations, the LDA model performed poorly. Examining the confusion matrix, we conclude that the model had an accuracy of only 74.1%, much lower than the logistic regression. The AUC score was also much lower, as you can see in \autoref{fig28}.
...
Reviewing the ROC curve, accuracy and AUC of the random forest model (\autoref{fig29}), it performed significantly better than LDA model, and marginally better than the LRM model. This model is especially good at predicting the "Yes" cases: those people who will actually open the deposit, with a sensitivity of 88%. However, this model performed relatively poorly on the overall accuracy on the test set, which was brought down by a poor specificity rate. This might be due to the fact that the proportion of "Yes" in the test set was very low.
...
\autoref{table2} shows a comparison of the performance of the models, along 4 metrics.
...
LR model 1    & 0.827       & 0.811          & 0.842         &            &              &              & 0.912 \\ \hline
LR model 2    & 0.824       & 0.796          & 0.852         & 0.828      & 0.8631       & 0.793        & 0.912 \\ \hline
LDA           & 0.741       & 0.658          & 0.824         & 0.828      & 0.827        & 0.636        & 0.805 \\ \hline
Random Forest & 0.854       & 0.884          & 0.825         & 0.828      & 0.879        & 0.825        & 0.923 \\ \hline
\end{tabular}
